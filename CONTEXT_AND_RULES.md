...
El proyecto se desarrolla bajo el directorio local del repositorio NEXUS (Windows: `C:\Users\marco\Documents\NEXUS`, macOS: `/Users/marc/Documents/Progrmas Propios/NEXUS`). En macOS, abrir con doble clic los `.command` del directorio raíz (`run_terminal.command`, `run_program.command`, etc.); los `.sh` equivalentes siguen disponibles para terminal. En Windows, usar los `.bat`. La arquitectura es modular y se divide estrictamente en los siguientes componentes:
