...
class NativeApiContractTests(unittest.TestCase):
    ...
    def test_exposes_history_ranking_attribution_and_news_linkage(
        ...
        self.assertIn("gold_upcoming", payload["calendar"])
        ...
        self.assertIn("demand", payload["gold"])
        self.assertFalse(payload["gold"]["demand"]["score_enabled"])
        self.assertEqual(payload["gold"]["demand"]["actual_etf_flows_status"], "STANDBY")
        self.assertEqual(payload["gold"]["outlook"]["private_sources_status"], "STANDBY")
